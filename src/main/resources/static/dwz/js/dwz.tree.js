    ...
    $.extend($.fn, {
        ...
        genTree: function (options) {
            ...
            return this.each(function () {
                ...
                var tvalue = node.children("a").attr("tvalue");
                if (authData) {
                    var authList = authData.split(",");
                    for (var i = 0; i < authList.length; i++) {
                        if (authList[i] == tvalue) {
                            checked = "checked";
                            break;
                        }
                    }
        ...
        _checkParent: function () {
            ...
            var rClass = (ckboxed == ckbox ? "indeterminate" : (ckboxed != 0 ? "checked" : "unchecked"));
            if (ckboxed != 0) {
                $(">div>.ckbox", parent).removeClass("unchecked").removeClass("checked").removeClass(rClass).addClass(aClass).find("input").attr("checked", "checked");
            }
            var $checkbox = $(parent);
